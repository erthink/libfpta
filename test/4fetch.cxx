...
TEST(Fetch, Invalid)
{
    ...
    EXPECT_EQ(nullptr,
              fptu_fetch(ro, space_exactly_noitems,
                         sizeof(space_exactly_noitems), fptu_max_fields));
    EXPECT_EQ(nullptr,
              fptu_fetch(ro, nullptr, sizeof(space_exactly_noitems), 0));
    EXPECT_EQ(nullptr, fptu_fetch(ro, space_exactly_noitems,
    ...
    EXPECT_EQ(nullptr,
              fptu_fetch(ro, space_exactly_noitems, 0, fptu_max_fields));
    EXPECT_EQ(nullptr,
    ...
    fptu_rw *pt;
    pt = fptu_fetch(ro, space_exactly_noitems, sizeof(space_exactly_noitems),
                    0);
    ...
    EXPECT_STREQ(nullptr, fptu_check(pt));
    pt =
        fptu_fetch(ro, space_maximum, sizeof(space_maximum), fptu_max_fields);
    ASSERT_NE(nullptr, pt);
    EXPECT_STREQ(nullptr, fptu_check(pt));
}
...
TEST(Fetch, Base)
{
    ...
    fetched_pt =
        fptu_fetch(origin_ro, fetched_space, sizeof(fetched_space), 0);
...
TEST(Fetch, Variate)
{
    ...
    for (auto fetch_buffer_space : space_cases) {
        ...
        for (auto more_items : items_cases) {
            ...
            origin_pt = fptu_init(origin_space, sizeof(origin_space),
                                  fptu_max_fields);
            ASSERT_NE(nullptr, origin_pt);
            EXPECT_STREQ(nullptr, fptu_check(origin_pt));
            origin_ro = fptu_take_noshrink(origin_pt);
            ...
            // check empty
            size_t origin_items =
                fptu_end_ro(origin_ro) - fptu_begin_ro(origin_ro);
            size_t origin_payload_bytes = origin_ro.total_bytes -
                                          units2bytes(origin_items) -
                                          fptu_unit_size;
            SCOPED_TRACE("origin.items " + std::to_string(origin_items) +
                         ", origin.payload_bytes " +
                         std::to_string(origin_payload_bytes) +
                         ", fetch.buffer_space " +
                         std::to_string(fetch_buffer_space) +
                         ", fetch.more_items " + std::to_string(more_items));
            fetched_pt =
                fptu_fetch(origin_ro, fetched_space, bytes, more_items);
            if (more_items > fptu_max_fields ||
                bytes < fptu_space(origin_items + more_items,
            ...
            EXPECT_GE(0, (int)fptu_check_and_get_buffer_size(
                             origin_ro, more_items, 0, nullptr));
            const char *error = "clean me";
            EXPECT_GE(bytes, fptu_check_and_get_buffer_size(
                                 origin_ro, more_items, 0, &error));
            ...
            for (unsigned n = 1; n < 11; ++n) {
                ...
                origin_ro = fptu_take_noshrink(origin_pt);
                ASSERT_STREQ(nullptr, fptu_check_ro(origin_ro));
                EXPECT_EQ(fptu_unit_size * (n + n + 1),
                          origin_ro.total_bytes);
