...
size_t fpt_space(size_t items, size_t data_bytes) {
    if (items > fpt_max_cols)
        items = fpt_max_cols;
    if (data_bytes > fpt_max_tuple_bytes)
        data_bytes = fpt_max_tuple_bytes;

    return sizeof(fpt_rw) + items * 4 + FPT_ALIGN_CEIL(data_bytes, 4);
}

fpt_rw* fpt_alloc(size_t items_limit, size_t data_bytes) {
    if (unlikely(items_limit > fpt_max_cols || data_bytes > fpt_max_tuple_bytes))
        return nullptr;
...
fpt_rw* fpt_init(void* space, size_t buffer_bytes, size_t items_limit) {
    if (unlikely(space == nullptr || items_limit > fpt_max_cols))
        return nullptr;

    if (unlikely(buffer_bytes < sizeof(fpt_rw) + 4 * items_limit))
        return nullptr;

    if (unlikely(buffer_bytes > FPT_ALIGN_CEIL(sizeof(fpt_rw) + fpt_max_tuple_bytes, 64)))
        return nullptr;

    fpt_rw *pt = (fpt_rw*) space;

    // LY: make a empty tuple
    pt->end = (buffer_bytes - sizeof(fpt_rw)) / 4 + 1;
