                                               ...
                                               const MDB_val *b)
{
    ...
    if (shortest >= sizeof(unsigned long)) {
        do {
            ...
            int diff =
                fptu_cmp2int(*(unsigned long *)pa, *(unsigned long *)pb);
            if (likely(diff))
                return diff;
                         ...
                         fpta_key &key, bool copy)
{
    ...
    case fptu_int64:
        if (unlikely(value.type == fpta_unsigned_int &&
                     value.uint > INT64_MAX))
            return FPTA_EVALUE;
                         ...
                         fpta_value &value)
{
    ...
    case fptu_opaque:
        value.type =
            (mdbx.mv_size > fpta_max_keylen) ? fpta_shoved : fpta_binary;
    ...
    case fptu_cstr:
        value.type =
            (mdbx.mv_size > fpta_max_keylen) ? fpta_shoved : fpta_string;
        value.binary_data = mdbx.mv_data;
        value.binary_length = mdbx.mv_size;
        return FPTA_SUCCESS;
