...
static __inline bool fpta_fk_changed(const fpta_table_schema *def,
                                     ...
                                     const fptu_ro &row_new, unsigned column)
{
                          ...
                          unsigned overstep)
{
    ...
    for (size_t i = 1; i < table_id->table.def->count; ++i) {
        if (fpta_shove2index(table_id->table.def->columns[i]) ==
            fpta_index_none)
            break;
        ...
        if (row_old.sys.iov_base) {
            ...
            fpta_key fk_key_old;
            rc = fpta_index_row2key(table_id->table.def->columns[i], i,
                                    row_old, fk_key_old, false);
            if (unlikely(rc != MDB_SUCCESS))
                return rc;

            rc = mdbx_del(txn->mdbx_txn, dbi[i], &fk_key_old.mdbx,
                          &pk_key_old);
            if (unlikely(rc != MDB_SUCCESS))
