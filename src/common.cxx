...
size_t fpt_field_units(const fpt_field* pf) {
    ...
    const void* payload = fpt_field_payload(pf);
    if (type == fpt_string) {
        // length is'nt stored, but zero terminated
        return (strlen((const char*) payload) + fpt_unit_size) >> 2;
    } else {
        // length is stored
        return ((fpt_varlen*) payload)->brutto + 1;
    }
}
