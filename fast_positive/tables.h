 ...
 * Если же опция ВКЛЮЧЕНА (определена как не 0), то при попытке такой
 * вставке или обновлении будет возвращена ошибка FPTA_EVALUE. */
#define FPTA_PROHIBIT_UPSERT_DENIL 0
 ...
 * данных, которые были штатно помещены и после прочитаны из из ftpa-таблицы,
 ...
 * при физическом отсутствии соответствующих колонок в строках таблиц.
 *
 ...
 *
 * Конкретные значения для каждого типа выбраны по следующим критериям:
 *  - безболезненность изъятия значения из оборота;
 *  - расположение с краю диапазона типа;
 *  - простота получения индексируемых представлений, при условии
 *    что NIL должен быть меньше всех не-NIL значений.
 *
 * Исходя из обозначенных критериев выбраны следующие замещающие значения:
 *  - INTxx_MIN для целых со знаком (int32_t, int64_t);
 *  - UINTxx_MAX для целых БЕЗ знака (uint16_t, uint32_t, uint64_t);
 *  - для плавающей точки "отрицательная тихая NaN-бесконечность",
 *    что в бинарном выглядит как "все единицы";
 *  - нуль для datetime, что соответствует нулю в UTC (1 января 1970);
 *  - "нули во всех байтах" для бинарных типов фиксированной длины;
 *  - для строк и всех типов переменной длины НЕТ designated empty,
 *    т.е. допустимы все значения;
 *
 * Поддержка IPv4:
 *  - IPv4 просит в качестве NIL значение 0, а не "все единицы",
 *    что расходится с текущим назначением UINT32_MAX для uint32_t.
 *  - для решения проблемы планируется добавить в fpta_value явный тип IPv4,
 *    с нулем в качестве designated empty.
 *
 ...
 *  - при получении индексированных представлений для беззнаковых типов
 *    производится вычитания соответствующего DENIL-а, таким образом шкала
 *    сдвигается делая "индексный" 0 (минимальное значение) из DENIL-значения,
 *    который соответствует NIL-у типа.
 *  - для строк и типов переменной длины из NIL-значений для индексирования
 *    формируется "супер ноль", проще говоря значения дополняются флагом.
...
#define FPTA_DENIL_FIXBIN_BYTE (0)

//#define FPTA_DENIL_UINT16 UINT16_MAX
//#define FPTA_DENIL_UINT32 UINT32_MAX
//#define FPTA_DENIL_UINT64 UINT64_MAX
//#define FPTA_DENIL_UINT FPTA_DENIL_UINT64
